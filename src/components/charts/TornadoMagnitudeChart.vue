...
const props = defineProps<{
  chartData: { magnitude: string; count: number; label?: string }[];
}>();
...
const chartConfig = computed(() => {
  ...
  const labels = props.chartData.map(
    (item) => item.label || `EF${item.magnitude}/F${item.magnitude}`
  );
        ...
        legend: {
          ...
          labels: {
            ...
            generateLabels: (chart: any) => {
              ...
              return originalLabels.map((label: string, i: number) => {
                return {
                  text: `${label} (${data[i]})`,
                  fillStyle: colors[i],
                  strokeStyle: "#ffffff",
